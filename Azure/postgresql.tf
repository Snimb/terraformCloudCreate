...
resource "azurerm_postgresql_flexible_server" "default" {
  ...
  name = lower("${random_pet.name_prefix.id}-${var.psql_name}")
  ...
  administrator_password = azurerm_key_vault_secret.secret_1.value
...
resource "azuread_group" "psql_ad_group" {
  ...
  display_name     = lower("${azurerm_postgresql_flexible_server.default.name}-${each.key}-${local.environment}")
  security_enabled = true
